...
function SoftEngine.Device:putPixel(x, y, z, color)
	local index = (math.floor(x) + math.floor(y) * self.workingWidth) + 1
...
function SoftEngine.Device:processScanLine(data, va, vb, vc, vd, color)
	...
	local gradient1 = (pa.y ~= pb.y) and (data.currentY - pa.y) / (pb.y - pa.y) or 1
	local gradient2 = (pc.y ~= pd.y) and (data.currentY - pc.y) / (pd.y - pc.y) or 1
...
function SoftEngine.Device:drawTriangle(v1, v2, v3, color)
	...
	local vnFace = v1.normal + v2.normal + v3.normal
	vnFace:scale(1 / 3)
	local centerPoint = v1.worldCoordinates + v2.worldCoordinates + v3.worldCoordinates
	centerPoint:scale(1 / 3);
	local lightPos = BABYLON.Vector3(0, 5, 10)
	local ndotl = self:computeNDotL(centerPoint, vnFace, lightPos)
