...
function SoftEngine.Mesh:computeFacesNormals()
	for k,currentFace in pairs(self.faces) do
		local vertexA = self.vertices[currentFace.A]
		local vertexB = self.vertices[currentFace.B]
		local vertexC = self.vertices[currentFace.C]
		
		self.faces[k].normal = vertexA.normal + vertexB.normal + vertexC.normal
		self.faces[k].normal:scale(1 / 3)
		self.faces[k].normal:normalize()
	end
end


--[[ Texture ]]--
SoftEngine.Texture = class()
function SoftEngine.Texture:init(filename)
	self:load(filename)
	print("Loaded texture", filename, self.width .. "x" .. self.height)
end

function SoftEngine.Texture:load(filename)
	self.buffer = love.image.newImageData(filename)
	self.width = self.buffer:getWidth()
	self.height = self.buffer:getHeight()
end

function SoftEngine.Texture:map(tu, tv)
	if self.buffer then
		local u = bit.brshift(math.floor(math.abs((tu * self.width) % self.width)), 0)
		local v = bit.brshift(math.floor(math.abs((tv * self.height) % self.height)), 0)
		
		local r, g, b, a = self.buffer:getPixel(u, v)
		
		return BABYLON.Color4(r / 255.0, g / 255.0, b / 255.0, a / 255.0)
	else
		print("Not found: ", self.buffer)
		return BABYLON.Color4(255, 255, 255, 255)
	end
end
...
function SoftEngine.Device:processScanLine(data, va, vb, vc, vd, color, texture)
	...
	for x = sx, ex - 1 do
		...
		if texture then
			...
			print("Not found: ", texture)
			textureColor = BABYLON.Color4(255, 255, 255, 255)
		end
		
		self:drawPoint(BABYLON.Vector3(x, data.currentY, z), BABYLON.Color4(color.r * ndotl * textureColor.r, color.g * ndotl * textureColor.g, color.b * ndotl * textureColor.b, 255))
	end
end
...
function SoftEngine.Device:drawTriangle(v1, v2, v3, color, texture)
	...
	if dP1P2 > dP1P3 then
		for y = bit.brshift(math.floor(p1.y), 0), bit.brshift(math.floor(p3.y), 0) do
			...
			if y < p2.y then
				...
				self:processScanLine(data, v1, v3, v1, v2, color, texture)
			else
				data.ndotla = nl1
                data.ndotlb = nl3
                data.ndotlc = nl2
				...
				self:processScanLine(data, v1, v3, v2, v3, color, texture)
			end
		end
	else
		for y = bit.brshift(math.floor(p1.y), 0), bit.brshift(math.floor(p3.y), 0) do
			...
			if y < p2.y then
				...
				self:processScanLine(data, v1, v2, v1, v3, color, texture)
			else
				data.ndotla = nl2
                data.ndotlb = nl3
                data.ndotlc = nl1
				...
				self:processScanLine(data, v2, v3, v1, v3, color, texture)
			end
		end
	end
end
...
function SoftEngine.Device:render(camera, meshes)
	...
	for _,cMesh in pairs(meshes) do
		...
		local worldView = worldMatrix * viewMatrix
		local transformMatrix = worldView * projectionMatrix
		
		for k,currentFace in pairs(cMesh.faces) do
			local transformedNormal = BABYLON.Vector3.TransformNormal(currentFace.normal, worldView)
			...
			if transformedNormal.z < 0 then
				...
				local color = 255
				self:drawTriangle(pixelA, pixelB, pixelC, BABYLON.Color4(color, color, color, 255), cMesh.texture)
			end
		end
	end
end
...
function SoftEngine.Device:createMeshesFromJSON(jsonData)
	...
	for materialIndex in pairs(jsonData.materials) do
		local material = {}
		
		material.name = jsonData.materials[materialIndex].name
		material.ID = jsonData.materials[materialIndex].id
		
		if jsonData.materials[materialIndex].diffuseTexture then
			material.diffuseTextureName = jsonData.materials[materialIndex].diffuseTexture.name
		end
	...
	for meshIndex in pairs(jsonData.meshes) do
		...
		for index = 1, #verticesArray, verticesStep do
			...
			if uvCount > 0 then
				local u = verticesArray[index + 6]
				local v = verticesArray[index + 7]
				mesh.vertices[verticeIndex].textureCoordinates = BABYLON.Vector2(u, v)
			else
				mesh.vertices[verticeIndex].textureCoordinates = BABYLON.Vector2(0, 0)
			end
		...
		if uvCount > 0 then
			local meshTextureID = jsonData.meshes[meshIndex].materialId
			local meshTextureName = materials[meshTextureID].diffuseTextureName
			mesh.texture = SoftEngine.Texture("textures/" .. meshTextureName)
		end
		
		mesh:computeFacesNormals()
		
		table.insert(meshes, mesh)
		print("Loaded model ", jsonData.meshes[meshIndex].name)
	end
